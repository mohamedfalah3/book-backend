...
// OAuth configuration
const SUCCESS_REDIRECT = 'https://book-backend-fmxe.onrender.com/auth/success';
const FAILURE_REDIRECT = 'https://book-backend-fmxe.onrender.com/auth/failure';
...
// Route to handle OAuth success redirect
app.get('/auth/success', async (req, res) => {
    try {
        const { userId, sessionId } = req.query;
        
        console.log('OAuth success received:', { userId, sessionId });
        
        if (userId && sessionId) {
            // OAuth was successful, create a secure session token
            const sessionToken = await createSecureSessionToken(userId, sessionId);
            
            // Redirect to app with success token using custom scheme
            const successUrl = `bookapp://auth?success=true&token=${encodeURIComponent(sessionToken)}&userId=${userId}`;
            res.redirect(successUrl);
            
        } else {
            // Missing parameters, redirect to failure
            const failureUrl = `bookapp://auth?success=false&error=missing_parameters`;
            res.redirect(failureUrl);
        }
        
    } catch (error) {
        console.error('Error handling OAuth success:', error);
        const failureUrl = `bookapp://auth?success=false&error=${encodeURIComponent(error.message)}`;
        res.redirect(failureUrl);
    }
});

// Route to handle OAuth failure redirect
app.get('/auth/failure', async (req, res) => {
    try {
        const { error } = req.query;
        
        console.log('OAuth failure received:', { error });
        
        // Redirect to app with failure
        const failureUrl = `bookapp://auth?success=false&error=${encodeURIComponent(error || 'OAuth authentication failed')}`;
        res.redirect(failureUrl);
        
    } catch (error) {
        console.error('Error handling OAuth failure:', error);
        const failureUrl = `bookapp://auth?success=false&error=${encodeURIComponent(error.message)}`;
        res.redirect(failureUrl);
    }
});
